...
[CreateAssetMenu(menuName = "A.I/State/Investigate Sound")]
    ...
    public override AIState Tick(AICharacterManager aiCharacter)
    {
        ...
        if (!destinationSet)
        {
            ...
            if (!IsDestinationReachable(aiCharacter, positionOfSound))
            {
                ...
                {
                    NavMeshPath patrialPath =  new NavMeshPath();
                    aiCharacter.navMeshAgent.CalculatePath(hit.position , patrialPath);
                    aiCharacter.navMeshAgent.SetPath(patrialPath);
                }
            }
            else
            {
